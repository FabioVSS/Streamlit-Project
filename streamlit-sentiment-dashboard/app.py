...
def plot_sentiment(airline):
    ...
    count = pd.DataFrame({'Sentiment':count.index,
                            'Tweets':count.values})
    return count
...
if len(choice)> 0:
    ...
    fig_0 = px.histogram(
                        ...
                        color_discrete_sequence=['#042B0B', '#3D553E' ,'#6D7F6E']
                        )
    fig_0.update_traces(
                        textfont_size=15,
                        textposition="outside",
...
if not st.sidebar.checkbox("Close", True, key='3'):
    ...
    processed_words = ' '.join([word for word in text.split() if 'http' not in word and not word.startswith('@') and word != 'RT'])
    wordcloud = WordCloud(
                            stopwords=STOPWORDS,
